...
__device__ u32 paging(uchar *buffer, u32 p, u32 offset) {
	...
	if(p_index == -1) {  //page fault!!
		...
		for(int i = 0; i < 4; i++) {
			storage[victim_p+i] = buffer[frame+i];
			buffer[frame+i] = storage[p];
		}
